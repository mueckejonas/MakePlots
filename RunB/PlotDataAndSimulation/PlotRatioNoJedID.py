...
dataInFileName = InDirectory+"PlotDijetSelection_Run2023B.root"
simInFileName = InDirectory+"_PlotSimulation_WithScale_Run32023_MC.root"
pdfnames = "PlotData(NoJedID)AndSimulation(JedID)_Run32023_"
...
for i in range(0,14):
    ...
    if JetNameArray[i] == "pt" or JetNameArray[i] == "mass":
        RootHisttoPdf(outDirectory+pdfnames+JetNameArray[i]+"1.pdf",data1,sim1,True,"#sigma [pb]",XaxisArray[i],"Run3 2023",JetNameArray[i]+" Jet1")
        RootHisttoPdf(outDirectory+pdfnames+JetNameArray[i]+"2.pdf",data2,sim2,True,"#sigma [pb]",XaxisArray[i],"Run3 2023",JetNameArray[i]+" Jet2")
        RootHisttoPdf(outDirectory+pdfnames+JetNameArray[i]+"3.pdf",data3,sim3,True,"#sigma [pb]",XaxisArray[i],"Run3 2023",JetNameArray[i]+" Jet3")
    else:
        RootHisttoPdf(outDirectory+pdfnames+JetNameArray[i]+"1.pdf",data1,sim1,False,"#sigma [pb]",XaxisArray[i],"Run3 2023",JetNameArray[i]+" Jet1")
        RootHisttoPdf(outDirectory+pdfnames+JetNameArray[i]+"2.pdf",data2,sim2,False,"#sigma [pb]",XaxisArray[i],"Run3 2023",JetNameArray[i]+" Jet2")
        RootHisttoPdf(outDirectory+pdfnames+JetNameArray[i]+"3.pdf",data3,sim3,False,"#sigma [pb]",XaxisArray[i],"Run3 2023",JetNameArray[i]+" Jet3")


#create yboost pdf
datayboost = dataKinematics.Get("data_yboost")
simyboost = simKinematics.Get("yboostsim_hist")
RootHisttoPdf(outDirectory+pdfnames+"yboostDataandSim.pdf",datayboost,simyboost,False,"#sigma [pb]","Yboost","Run3 2023","Yboost")
#create chi pdf
datachi = dataKinematics.Get("data_chi")
simchi = simKinematics.Get("chisim_hist")
RootHisttoPdf(outDirectory+pdfnames+"chiDataandSim.pdf",datachi,simchi,False,"#sigma [pb]","Chi","Run3 2023","Chi")
#create mjj pdf
datamjj = dataKinematics.Get("data_mjj")
simmjj = simKinematics.Get("mjjsim_hist")
RootHisttoPdf(outDirectory+pdfnames+"mjjDataandSim.pdf",datamjj,simmjj,True,"#sigma [pb]","Mjj [GeV]","Run3 2023","Mjj")
