...
pdfnames = "PlotJetKinematics_Run2023C_"
inFileName = inDirectory+"PlotJetKinematics_Run2023C.root"
...
for i in range(0,14):
    ...
    if JetNameArray[i] == "pt" or JetNameArray[i] == "mass":
        RootHisttoPdf(outDirectory+pdfnames+JetNameArray[i]+".pdf",data1,True,3,"N",XaxisArray[i],"Run3 C 2023 "+LegendArray[i],data2,data3)
    else:
        RootHisttoPdf(outDirectory+pdfnames+JetNameArray[i]+".pdf",data1,False,3,"N",XaxisArray[i],"Run3 C 2023 "+LegendArray[i],data2,data3)

#create yboost pdf
yboostData = Kinematics.Get("data_yboost")
RootHisttoPdf(outDirectory+pdfnames+"yboost.pdf",yboostData,False,1,"N","Y_{boost}","Y_{boost} RunC 3 2023")
#create chi pdf
chiData = Kinematics.Get("data_chi")
RootHisttoPdf(outDirectory+pdfnames+"chi.pdf",chiData,False,1,"N","#chi_{dijet}","#chi_{dijet} RunC 3 2023")
#create mjj pdf
mjjData = Kinematics.Get("data_mjj")
RootHisttoPdf(outDirectory+pdfnames+"mjj.pdf",mjjData,True,1,"N","M_{jj} [GeV]","M_{jj} RunC 3 2023")
